...
const LineChartHochbeet = ({
  ...
  useEffect(() => {
    ...
    const minDatasetValue = Array.isArray(lineData[0])
      ...
      : Math.min(...lineData.map((dataPoint) => parseFloat(dataPoint.value)));

    // setting up default values, which in some cases will be overwritten
    const yAxisReverseEnabled =
      lineChartConfig.options?.scales?.y?.reverse || false;
            ...
            ticks: {
              ...
              callback: function (value) {
                ...
                if (lineChartConfig.options?.scales?.y?.ticks?.callback) {
                  return lineChartConfig.options.scales.y.ticks.callback(
                    formattedValue
                  );
                }
...
export default LineChartHochbeet;
